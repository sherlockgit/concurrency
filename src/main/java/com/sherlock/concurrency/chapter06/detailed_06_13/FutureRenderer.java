 ...
 *       不能边下载边渲染。
 *
 * FutureRenderer使用了两个任务，其中一个负责渲染文本，另一个负责下载图像。
 * * 如果渲染文本的速度远远高于下载图像的速度(可能性很大)，那么程序的最终性能
 * * 与串行执行时的性能差别不大，而代码却变得更复杂了。当使用两个线程时，至多能
 * * 将速度提高一倍。因此，虽然做了许多工作来并发执行异构任务以提高并发度，但从中
 * * 获得的并发性却是十分有限的。
 */
